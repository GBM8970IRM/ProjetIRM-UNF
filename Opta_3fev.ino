...
#define SLAVE_ID 4097  // Remplace par l'ID de ton capteur
#define TEMP_REGISTER 0x0000  // Adresse de la température
#define HUM_REGISTER  0x0001  // Adresse de l'humidité
...
void loop() {
  ...
  //Impression de la temperature de l'eau
  Serial.print("TemperatureEauEntrée (°C): ");
  Serial.println(temp1);
  Serial.print("TemperatureEauSortie (°C): ");
  Serial.println(temp2);

  //Lecture des valeurs du thermometre Sensecap S-TH-01 sur le Modbus (RS485) de Opta
  //tempIRM, humIRM = RS485.read();
  //Serial.println(tempIRM/100);
  //Serial.println(humIRM/100);

  // Lire la température (registre 0x0000)
  if (!ModbusRTUClient.requestFrom(SLAVE_ID, INPUT_REGISTERS, TEMP_REGISTER, 1)) {
    Serial.println("Erreur : impossible de lire la température !");
    }
  else {
    uint16_t rawTemp = ModbusRTUClient.read();  
    tempIRM = rawTemp / 100;  // Diviser si nécessaire
    Serial.print("Température : ");
    Serial.print(tempIRM);
    Serial.println(" °C");
  }

  // Lire l'humidité (registre 0x0001)
  if (!ModbusRTUClient.requestFrom(SLAVE_ID, INPUT_REGISTERS, HUM_REGISTER, 1)) {
    Serial.println("Erreur : impossible de lire l'humidité !");
  } 
  else {
    uint16_t rawHum = ModbusRTUClient.read();  
    humIRM = rawHum / 100;  // Diviser si nécessaire
